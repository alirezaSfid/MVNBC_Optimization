...
mkdir -p logs200points
echo "Logs will be saved in:" 
echo "$JULIA_PROJECT/scripts/logs200points"
...
# for R in 28; do
    ...
    # for r in 0.05; do
        for P in "${P_values[@]}"; do
            ...
            srun --exclusive -n 1 --cpus-per-task=12 julia --project=$JULIA_PROJECT ../code/NW_medium.jl $R $r "$P" > "logs200points/output_R=${R}_r=${r}_P=${P//,/}.log" 2>&1 &
